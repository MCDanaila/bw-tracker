...
export default function FoodDatabasePage() {
  const { isCoach } = useAthleteContext();
    ...
    <div className="space-y-4">
      ...
      {!isLoading && foods.length === 0 && !search && !unitFilter && !stateFilter ? (
        <EmptyState
          ...
          title="No foods yet"
          description={isCoach ? 'Add your first food to get started.' : 'No foods have been added yet.'}
          action={isCoach ? { label: 'Add Food', onClick: () => setShowAddDialog(true) } : undefined}
        />
      ) : (
        <DataTable
          columns={columns}
