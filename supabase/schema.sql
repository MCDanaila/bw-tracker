...
CREATE TABLE daily_logs (
    ...
    sleep_quality NUMERIC CHECK (sleep_quality >= 1 AND sleep_quality <= 5),
    ...
    gym_energy NUMERIC CHECK (gym_energy >= 1 AND gym_energy <= 5),
    gym_mood NUMERIC CHECK (gym_mood >= 1 AND gym_mood <= 5),
    soreness_level NUMERIC CHECK (soreness_level >= 1 AND soreness_level <= 5),
    ...
    digestion_rating INTEGER CHECK (digestion_rating >= 1 AND digestion_rating <= 5),
    digestion_comments TEXT,
    bathroom_visits INTEGER,
    stress_level NUMERIC CHECK (stress_level >= 1 AND stress_level <= 5),
    daily_energy NUMERIC CHECK (daily_energy >= 1 AND daily_energy <= 5),
